...
public sealed class AC3Solver {
	...
	public IEnumerable<Solution> Solutions() {
		...
		IEnumerable<IEnumerable<KeyValuePair<IVariable, object>>> combos = new[]
			{ Enumerable.Empty<KeyValuePair<IVariable, object>>() };
	...
	private void EnsureArcConsistency() {
		while (_agenda.Count != 0) {
			...
			for (var i = dfrom.Count - 1; i >= 0; i--) {
				var someSatisfied = dto.Any(vOther => {
					var assign = new Assignment(new Dictionary<IVariable, object> {
						{ arc.From, dfrom[i] },
						{ arc.To, vOther }
					});
